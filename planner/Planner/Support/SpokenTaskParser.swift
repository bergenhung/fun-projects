...
/// Extracts a spoken time (e.g. "meeting at 3pm") from a voice-entry transcript,
/// using Foundation's date detector rather than hand-rolled time parsing.
enum SpokenTaskParser {
    ...
    static func parse(_ transcript: String) -> Result {
        ...
        guard !trimmed.isEmpty else { return Result(title: "", hour: nil) }

        guard let detector = try? NSDataDetector(types: NSTextCheckingResult.CheckingType.date.rawValue) else {
            return Result(title: trimmed, hour: nil)
        }
        ...
        return Result(title: title, hour: clampedHour)
    }
}
